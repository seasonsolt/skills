...
核心入口是 [`SKILL.md`](SKILL.md) 及同目录 `references/`（诊断、定位、选题、文案、配图、草稿、发布与去 AI 味）。
...
- **Codex / 其它无斜杠命令的 agent**：使用同一发行包内的 [`humanizer-zh`](../humanizer-zh/SKILL.md)，按其 **24 项 AI 痕迹 + 50 分 rubric** 改写并打分，**≥40/50 才算完成**，不到就继续改，并向用户报一句"改了什么 + 分数"。
...
- **`posts/`** 是用户私人发布归档：**绝不进任何公开仓库、不外发**、改写前先读别乱删。是否提交到使用者自己的私有仓库由使用者决定。
- **真实性**：文案里的"亲测/我的用法"必须真；转载标清出处，别包装成原创。

## 提交约定

- 默认在分支上改；`.env` 永不提交，`posts/` 绝不进任何公开仓库；commit 说明用中文、讲清价值。
- `../humanizer-zh/` 是 vendored（op7418/Humanizer-zh，MIT），保留其 LICENSE 与出处。

## 公开协作

本目录是 [`seasonsolt/skills`](https://github.com/seasonsolt/skills) 中可直接安装、测试与贡献的公开发行源码。提交改进时保留 `assets/`、`examples/`、`references/` 与 `humanizer-zh` 的相对路径自洽；`posts/`、`.env` 等私人内容永不外发。
